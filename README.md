...
[![Live Demo](https://img.shields.io/badge/🔴_Live_Demo-Online-ff3366?style=for-the-badge)](https://genai-49nr.vercel.app/)
...
### 🔗 [**⚡ LAUNCH THE LIVE PLATFORM ⚡**](https://genai-49nr.vercel.app/)

**`https://genai-49nr.vercel.app/`**
...
It's an autonomous work agent that holds full context on your projects, tasks, documents, research, and past decisions — then takes a request, **synthesizes an explicit reasoning trace, plans it as a multi-step sequence, executes each step with the right tool, and asks for your approval before anything consequential happens.** Every run is fully inspectable: the intent deconstruction, the step plan, the tools invoked, the token usage, the cost, the latency, and the verification step are all logged, not hidden behind a single opaque reply.
...
User Prompt / Scheduled Trigger
            │
            ▼
  🧠 Gen Thinking Trace  ──▶  intent deconstruction · context recall · safety gate
            │
   🗺️ Step Plan  ──▶  explicit, numbered, inspectable
            │
   ┌────────┼─────────┬─────────────┬──────────────┐
   ▼        ▼          ▼             ▼              ▼
...
### 🧠 Transparent Agent Runs with Visible Thinking Trace
Every request first synthesizes a readable Gen reasoning trace — intent deconstruction, context and constraint recall, a safety/policy check for elevated actions, tool routing, and the verification standard being applied — before producing a structured `AgentRun` with per-step status (`pending → running → completed → failed → waiting_approval`), tools used, token usage, estimated cost, latency, and suggested next actions.
...
Actions above a risk threshold — like a production deploy or branch merge — pause and surface an `Approval` request with action, target, change count, and full details before proceeding. Approve or reject with notes; the decision is permanently logged.
...
flowchart TD
    ...
    B --> E[Gen Thinking Trace<br/>intent + context + safety gate]
    D --> E
    E --> F[Explicit Step Plan]
...
git clone https://github.com/sathi2305/GENAI.git
cd GENAI
...
**Base URL:** `https://genai-49nr.vercel.app/api/v1`

### Agent
| Method | Endpoint | Description |
|---|---|---|
| `POST` | `/agent/chat` | Send a prompt — runs the full reasoning/plan/tool/approval pipeline |
...
└── src/
    ...
    ├── server/
    │   ├── agent.ts                   # Multi-model reasoning trace, planner & tool execution
...
**Live URL:** https://genai-49nr.vercel.app/
...
- [ ] Graceful client-side degradation when the agent endpoint errors, instead of surfacing raw error text
...
**[🧬 Try the Live Platform](https://genai-49nr.vercel.app/)**
